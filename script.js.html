<meta name='viewport' content='width=device-width, initial-scale=1'/>// script.js - সমস্ত ফাংশন এবং লজিক এই ফাইলে থাকবে
let irLearningIndex = -1; 
...
function loadStates() { 
    ...
    .then(data => { 
        for (let i = 0; i < 5; i++) { 
            ...
            if (lightCheckbox) { 
                lightCheckbox.checked = data.lights[i].state; 
            } 
        ...
        if (data.gasDetected) { 
            gasAlertDiv.style.display = 'block'; 
        } else { 
            gasAlertDiv.style.display = 'none'; 
        } 
...
function toggleLight(index) { 
    ...
    .then(state => { 
        document.querySelector(`#lightsCard .device-container:nth-child(${index + 1}) .switch input[type="checkbox"]`).checked = state === '1'; 
    }) 
...
function updateRGB() { 
    ...
    if (document.getElementById('rgbPower').checked) { 
        toggleRGBPower(); 
    } 
    updateRGBPreview(); 
} 
...
function updateAdminCredentials() { 
    ...
    if (newUsername.length < 3 || newPassword.length < 3) { 
        statusElement.textContent = "Username and password must be at least 3 characters long."; 
    ...
    .then(response => { 
        if (response.ok) { 
            ...
            setTimeout(() => { 
                alert("Credentials updated. Please refresh the page to log in with new credentials."); 
...
function updateWiFiCredentials() { 
    ...
    if (newSsid.length === 0) { 
        statusElement.textContent = "WiFi SSID cannot be empty."; 
        statusElement.style.color = "orange"; 
        return; 
    } 
    if (newPassword.length > 0 && newPassword.length < 8) { 
        statusElement.textContent = "WiFi password should be at least 8 characters long (or leave empty for open network)."; 
        statusElement.style.color = "orange"; 
        return; 
    } 
    ...
    .then(response => { 
        if (response.ok) { 
            ...
            setTimeout(() => { 
                hideLoading(); 
                alert("WiFi credentials updated. The ESP32 is restarting and attempting to connect to the new network. Please wait a moment and refresh this page, or try accessing the device at its new IP address if it changed."); 
                window.location.reload(); 
            }, 5000); 
        } else { 
            return response.text().then(text => { throw new Error(text); }); 
    ...
    .catch(error => { 
        ...
        statusElement.textContent = `Error: ${error.message}. Please check credentials.`; 
        statusElement.style.color = "red"; 
    }); 
} 
...
function updateDeviceName(deviceType, index = -1, fromModal = false) { 
    ...
    let deviceId; 
    let inputElementId; 
    let modalInputElementId = fromModal ? `${deviceType}${index}_newName_modal` : ''; 
    if (deviceType === 'light') { 
        inputElementId = `light${index}_newName`; 
        newName = document.getElementById(inputElementId).value; 
        deviceId = `lightName${index}`; 
    } else if (deviceType === 'ac') { 
        inputElementId = fromModal ? modalInputElementId : `ac${index}_newName`; 
        newName = document.getElementById(inputElementId).value; 
        deviceId = `acName${index}`; 
    } else if (deviceType === 'tv') { 
        inputElementId = 'tv_newName'; 
        newName = document.getElementById(inputElementId).value; 
        deviceId = 'tvName'; 
    } else if (deviceType === 'fan') { 
        inputElementId = 'fan_newName'; 
        newName = document.getElementById(inputElementId).value; 
        deviceId = 'fanName'; 
    } else if (deviceType === 'rgbLight') { 
        inputElementId = 'rgbLight_newName'; 
        newName = document.getElementById(inputElementId).value; 
        deviceId = 'rgbLightName'; 
    } else { 
    ...
    .then(response => { 
        if (response.ok) { 
            ...
            if (fromModal) { 
                document.getElementById(`acRemoteTitle${index}`).textContent = newName; 
            } else { 
                document.getElementById(`${inputElementId.replace('_newName', '_name_change_controls')}`).style.display = 'none'; 
            } 
...
function updateIrCodeDisplay() { 
    ...
    .then(data => { 
        if (data.irLearningActive) { 
            irStatusText.textContent = `IR Receiver is actively listening. Learning for Light ${data.irLearningIndex !== -1 ? data.irLearningIndex : 'None'}. Press a button on your remote.`; 
            irStatusText.style.color = '#ffc107'; 
        } else { 
            irStatusText.textContent = "IR Receiver is actively listening for signals."; 
            irStatusText.style.color = '#ccc'; 
        } 
        if (data.irCode && data.irCode !== "0x0") { 
            irCodeDisplay.textContent = data.irCode; 
        } else { 
            irCodeDisplay.textContent = "No signal detect."; 
        } 
        ...
        for (let i = 0; i < 5; i++) { 
            ...
            if (learnButton) { 
                if (data.irLearningActive && data.irLearningIndex == i) { 
                    learnButton.classList.add('learning'); 
                    learnButton.textContent = 'Learning...'; 
                } else { 
                    learnButton.classList.remove('learning'); 
                    learnButton.textContent = 'Learn IR'; 
                } 
            } 
        } 
    }) 
    .catch(error => { 
        console.error('Error fetching IR code:', error); 
        irCodeDisplay.textContent = "Error fetching IR status."; 
        irCodeDisplay.style.color = 'red'; 
        irStatusText.textContent = `Connection error for IR receiver: ${error.message}`; 
        irStatusText.style.color = 'red'; 
    }); 
} 
...
function startIrLearning(index) { 
    ...
    .then(response => { 
        if (response.ok) { 
            document.getElementById('irLearningStatus').textContent = `Press remote button for Light ${index}.`; 
            document.getElementById('irLearningStatus').style.color = '#ffc107'; 
            for (let i = 0; i < 5; i++) { 
    ...
    .catch(error => { 
        ...
        document.getElementById('irLearningStatus').textContent = `Error starting learning: ${error.message}`; 
        document.getElementById('irLearningStatus').style.color = 'red'; 
        irLearningIndex = -1; 
        for (let i = 0; i < 5; i++) { 
            const btn = document.getElementById(`learnIr${i}`); 
            if (btn) { 
...
function resetIrCode(index) { 
    if (confirm(`Are you sure you want to reset the IR code for Light ${index}? This cannot be undone without re-learning.`)) { 
        ...
        .then(response => { 
            if (response.ok) { 
                alert(`IR code for Light ${index} reset successfully.`); 
                updateIrCodeDisplay(); 
            } else { 
                return response.text().then(text => { throw new Error(text); }); 
            } 
        ...
        .catch(error => { 
            console.error('Error resetting IR code:', error); 
            alert(`Error resetting IR code: ${error.message}`); 
        }); 
    } 
} 
...
// === ADDED FUNCTIONS FOR GRID NAMES ===
const gridButtonsConfig = [
    { id: 'Lights', default: 'LIGHTS' },
    { id: 'Rgb', default: 'RGB' },
    { id: 'Ac', default: 'AC' },
    { id: 'Appliances', default: 'APPLIANCES' },
    { id: 'IrHub', default: 'IR HUB' },
    { id: 'IntelliSync', default: 'INTELLISYNC' },
    { id: 'Sawt', default: 'SAWT' },
    { id: 'Current', default: 'CURRENT' },
    { id: 'RgbAnimation', default: 'RGB ANIMATION' },
    { id: 'MotionSensor', default: 'MOTION SENSOR' }
];
...
function loadGridNames() {
    gridButtonsConfig.forEach(button => {
        ...
        if (inputElement) {
            inputElement.value = displayName; // Set the value for the settings input
        }
    });
}
...
function saveGridNames() {
    ...
    try {
        gridButtonsConfig.forEach(button => {
            ...
            const displayName = newName || button.default; // Revert to default if empty

            localStorage.setItem(storageKey, displayName); // Save to local storage
            document.getElementById(labelId).textContent = displayName; // Update main grid
            document.getElementById(inputId).value = displayName; // Update settings input
        });
